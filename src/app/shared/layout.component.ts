      ...
      <aside class="w-64 bg-white border-r border-slate-200 hidden md:flex flex-col">
        ...
        <nav class="flex-1 p-4 space-y-2">
          ...
          <ng-container *ngIf="authService.currentUser()?.role === 'SUPER_ADMIN'">
            <a routerLink="/super-admin/admins" routerLinkActive="bg-primary-50 text-primary-600"
              class="flex items-center space-x-3 p-3 rounded-lg text-slate-600 hover:bg-slate-50 transition-colors">
          ...
          <ng-container *ngIf="authService.currentUser()?.role === 'ADMIN' || authService.currentUser()?.role === 'SUPER_ADMIN'">
            <a routerLink="/admin/approvals" routerLinkActive="bg-primary-50 text-primary-600"
              class="flex items-center space-x-3 p-3 rounded-lg text-slate-600 hover:bg-slate-50 transition-colors">
          ...
          <a *ngIf="authService.currentUser()?.role !== 'SUPER_ADMIN'" routerLink="/plots/create" routerLinkActive="bg-primary-50 text-primary-600"
            class="flex items-center space-x-3 p-3 rounded-lg text-slate-600 hover:bg-slate-50 transition-colors">
        ...
        <header class="bg-white border-b border-slate-200 h-16 flex items-center justify-between px-8">
          <div class="flex items-center space-x-4">
             ...
             <span class="px-2 py-1 rounded text-xs font-medium" 
                   [ngClass]="{
                     'bg-purple-100 text-purple-700': authService.currentUser()?.role === 'SUPER_ADMIN',
                     'bg-indigo-100 text-indigo-700': authService.currentUser()?.role === 'ADMIN',
                     'bg-blue-100 text-blue-700': authService.currentUser()?.role === 'USER'
                   }">
               {{ authService.currentUser()?.role?.replace('_', ' ') }}
             </span>
          </div>
