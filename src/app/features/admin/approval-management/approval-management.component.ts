...
@Component({
  ...
  template: `
    <div class="approval-wrapper animate-fade-in">
      <div class="mb-5 d-flex justify-content-between align-items-center px-3">
        <div>
          <h1 class="h2 fw-bold tracking-tight mb-1 text-primary">
            Asset Verification
          </h1>
          <p class="text-slate-500 mb-0">
            System-wide review queue for property tokenization requests.
          </p>
        </div>
        <div class="d-flex gap-3">
          <div
            class="status-indicator-group d-flex gap-4 p-3 bg-white dark:bg-slate-900 rounded-4 border dark:border-slate-800 shadow-sm"
          >
            <div class="data-point">
              <div class="label uppercase">Pending</div>
              <div
                class="val fw-bold d-flex align-items-center gap-2 text-slate-900 dark:text-white"
              >
                {{ pendingPlots.length }}
                <span class="count-badge warning">ACT</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <mat-tab-group class="custom-tabs">
        ...
        <mat-tab label="Awaiting Review">
          <div class="tab-content py-4 px-3">
            <div
              *ngIf="pendingPlots.length === 0"
              class="empty-state-v2 dark:bg-slate-900 dark:border-slate-800"
            >
              <div class="empty-icon-circle bg-slate-50 dark:bg-slate-800">
                <mat-icon class="text-slate-300">fact_check</mat-icon>
              </div>
              <h4 class="fw-bold text-slate-900 dark:text-white">
                Review Queue Clear
              </h4>
              <p class="text-slate-500">
                No new property plots are currently awaiting verification.
              </p>
            </div>
            ...
            <div class="row g-4">
              <div class="col-md-6 col-xl-4" *ngFor="let plot of pendingPlots">
                <mat-card
                  class="verification-card-premium border-0 shadow-sm overflow-hidden h-100 dark:bg-slate-900 dark:border dark:border-slate-800"
                >
                  ...
                  <mat-card-content class="p-4">
                    ...
                    >
                      <h3
                        class="h6 fw-bold mb-0 text-slate-900 dark:text-white"
                      >
                        {{ plot.plotName }}
                      </h3>
                    ...
                    <div
                      class="d-flex align-items-center text-slate-400 small mb-4"
                    >
                    ...
                    <div
                      class="d-flex gap-2 pt-3 border-top dark:border-slate-800"
                    >
                      <button
                        mat-flat-button
                        color="primary"
          ...
          <div class="tab-content py-4 px-3">
            <div class="row g-3">
              <div class="col-12" *ngFor="let plot of approvedPlots">
                <mat-card
                  class="verified-asset-row border-0 shadow-sm dark:bg-slate-900 dark:border dark:border-slate-800"
                >
                  <mat-card-content
                  ...
                  >
                    <div class="d-flex align-items-center gap-3">
                      ...
                      <div>
                        <div class="fw-bold text-slate-900 dark:text-white">
                          {{ plot.plotName }}
                        </div>
                        <div class="tiny text-slate-500">
                          {{ plot.address }}
                        </div>
                      </div>
                    </div>
                    ...
                    <div class="d-flex align-items-center gap-4">
                      <div class="text-end d-none d-md-block">
                        <div class="tiny text-slate-400 uppercase fw-bold">
                          Current Value
                        </div>
                        <div class="small fw-bold text-indigo">
                      ...
                      >
                        <button
                          *ngIf="plot.status === 'approved'"
                          mat-flat-button
                          color="accent"
                          class="rounded-pill tiny fw-bold"
                          (click)="mintNft(plot)"
                        ...
                        <span
                          *ngIf="plot.status === 'minted'"
                          class="text-success tiny fw-bold uppercase d-flex align-items-center gap-1"
                        >
          ...
          <div class="tab-content py-4 px-3">
            <div class="row g-3">
              <div class="col-12" *ngFor="let plot of rejectedPlots">
                <mat-card
                  class="rejected-card border-0 border-start border-danger border-4 shadow-sm dark:bg-slate-900"
                >
                  <mat-card-content
                    class="p-4 d-flex justify-content-between align-items-center"
                  >
                    <div>
                      <h5 class="fw-bold text-slate-900 dark:text-white mb-1">
                        {{ plot.plotName }}
                      </h5>
                      <div class="text-danger small fw-medium mb-2">
                        <mat-icon class="tiny-icon align-middle"
                          >error_outline</mat-icon
                        >
                        {{ plot.rejectionReason }}
                      </div>
                      <div class="tiny text-slate-400">
                        Processed on {{ plot.createdAt | date }}
                      </div>
                    </div>
                    <button
                      mat-stroked-button
                      class="rounded-pill tiny fw-bold text-slate-700 dark:text-slate-300"
  ...
  styles: [
    `
      ...
      /* Tabs */
      .custom-tabs ::ng-deep .mat-mdc-tab-label-container {
        border-bottom: 1px solid var(--border-color);
      }
      .custom-tabs ::ng-deep .mat-mdc-tab .mdc-tab__text-label {
        font-family: 'Inter';
        font-weight: 600;
        font-size: 0.875rem;
        color: var(--text-secondary);
      }
      .custom-tabs ::ng-deep .mat-mdc-tab.mdc-tab--active .mdc-tab__text-label {
...
export class ApprovalManagementComponent implements OnInit {
  ...
  approvePlot(plot: Plot) {
    this.plotService.approvePlot(plot.id).subscribe({
      next: () => {
        ...
        this.snackBar.open(
          `Plot "${plot.plotName}" approved successfully!`,
          'Close',
          { duration: 3000 },
        );
      },
    ...
    if (reason) {
      this.plotService.rejectPlot(plot.id, reason).subscribe({
        next: () => {
          ...
          this.snackBar.open(`Plot "${plot.plotName}" rejected.`, 'Close', {
            duration: 3000,
          });
        },
        error: () =>
  ...
  mintNft(plot: Plot) {
    ...
    this.snackBar.open(
      `Initiating NFT Minting for ${plot.plotName}...`,
      'Close',
      { duration: 2000 },
    );
    ...
    this.nftService.mintNft(plot.id).subscribe({
      ...
      error: (err: any) => {
        ...
        this.snackBar.open(
          'Minting failed: ' +
            (err.error?.message || 'Check wallet connection'),
          'Close',
          { duration: 3000 },
        );
      },
